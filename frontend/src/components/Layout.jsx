...
const Trip = () => {
      ...
      autocompleteServiceRef.current?.getPlacePredictions(
        {
          input: value,
          types: ['(cities)'],
          componentRestrictions: { country: 'in' }, // Optional: restrict to specific country
        },
        (predictions, status) => {
          if (status === 'OK') {
  ...
  const handleSelectPlace = (placeDescription) => {
    setPlace(placeDescription);
    handleInputChange("location", placeDescription);
    setShowSuggestions(false);
    
    // If you need more details about the place, you can use the PlacesService
    // This requires a Place ID which we don't have from the predictions
    // You might need to modify the code to get more details if needed
  };
              ...
              <div className="absolute z-10 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg">
                {predictions.map((prediction, index) => (
                  <div
                    ...
                    onClick={() => handleSelectPlace(prediction.description)}
                  >
